function newfns = normalize_beads(rinput,fns)
...
%     filenames=strcat(data_dir,f);
    [path, ~, ~] = fileparts(fns{1});
    filenames = fns;
    data_dir = [path filesep];
    num_files = length(fns);
    for i =1:length(filenames)
        t = regexp(filenames{i}, '\', 'split');
        f{i} = t{ end };
    end
...
%             figure;plot(file.bead_data(:,1),file.bead_data(:,2:6),'.');xlabel('time');ylabel('intensity');title([file.header.filename  'raw beads'])
%             print(gcf,'-dpng',[ file.header.filename  'raw beads' '.png'])
%             figure;plot(file.bead_data,'.');xlabel('time');ylabel('intensity');title([ file.header.filename  'beads的第一次平滑'])
            file=file.smooth_beads(bead_window);
%             figure;plot(file.smoothed_time,file.smoothed_beads,'.');xlabel('time');ylabel('intensity');title([ file.header.filename  'beads的平滑'])
%             print(gcf,'-dpng',[ file.header.filename  'beads的平滑' '.png'])   %保存为png格式的图片到当前路径
            file=file.compute_bead_slopes(bead_means);
%             figure;plot(file.smoothed_time,file.bead_slopes,'.');xlabel('time');ylabel('slope');title([ file.header.filename   '根据beads计算出的斜率'])
%             print(gcf,'-dpng',[ file.header.filename  '根据beads计算出的斜率' '.png'])
            file=file.make_times_unique;
%             figure;plot(file.data(:,1),file.data(:,[7 12 15 16 17]),'.');xlabel('time');ylabel('intensity');title([ file.header.filename '其他marker校正前'])
%             print(gcf,'-dpng',[ file.header.filename  '其他marker校正前' '.png'])
            file=file.correct_channels;  %changes values of file.data
%             figure;plot(file.data(:,1),file.data(:,[7 12 15 16 17]),'.');xlabel('time');ylabel('intensity');title([ file.header.filename '其他marker校正后'])
%             print(gcf,'-dpng',[ file.header.filename  '其他marker校正后' '.png'])
...
%             figure;plot(file.smoothed_time,file.smoothed_beads,'.');xlabel('time');ylabel('intensity');title([file.header.filename 'beads校正后平滑'])
%             print(gcf,'-dpng',[ file.header.filename  'beads校正后平滑' '.png'])
...
%             fca_writefcs([norm_dir f{i}(1:end-4) '_normalized.fcs'],file.data,file.markernames,file.channelnames,file.header);
            temp_name = [norm_dir 'beadsnormalized_' f{i} '_' date];
