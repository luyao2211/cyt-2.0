...
% varargin   command line arguments to cyt (see VARARGIN)
    ...
    case1PlotTypes = {...	% plot options for a single selected channel
          'Histograms by Gates',	@(by_gate)plot_histograms(1);
          'Histograms',             @(by_gate)plot_histograms(0);
          'Plot significance test',          @plot_significance_test;
...
%     line()
    ...
    set(gca, 'xtick', 1:length(selectedGates));

        x_labs = strcat(...
            gates(selectedGates, 1)...
            );            
    set(gca, 'Xticklabel', x_labs);
...
%                         'Sample', 1, {num2str(min(2000, floor(numel(gateContext)*.20)))});
    ...
    for i=selected_gates
        ...
        % channel name from the first selected gate
            ...
            prev_selected = get(handles.lstChannels, 'Value');
            [cofactor,selectedChannels, isDnagate, isSaveout, isOriginal, isPrefix, strPrefix] = ...
                Preprocess('selectchannels', gates{i, 3},prev_selected);
            if cofactor == 0
                return;
            end
...
handles = gethand;
    ...
    regexps{end+1, 1} = [date new_regexp];
    regexps{end, 2} = new_regexp;
	put('regexps', regexps);
